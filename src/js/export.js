...
var exportFunc = execMain(function () {
    ...
    var outCustomServ = $('<a class="click"/>').html('Export to URL').click(uploadDataCustomServ);
        ...
        for (var ff = 0; ff < files.length; ff++) {
            ...
            msgf.push(
                ff +
                    ...
                    '. ' +
                    msgfmt
                        .replace('%s', nsolv || (size ? Math.ceil(size / 1024) + ' KB' : 'N/A'))
                    ...
                    .then(function (val) {
                        ...
                        for (var i = 0; i < keys.length; i++) {
                            ...
                            Array.prototype.push.apply(
                                arrs[i],
                                JSON.parse(LZString.decompressFromEncodedURIComponent(datas[i]))
                            );
                        }
                        return baseObj;
                    });
    ...
    function showExportDiv() {
        updateExpString().then(function () {
            if (window.Blob) {
                ...
                outFile.attr(
                    'download',
                    'cstimer_' + mathlib.time2str(new Date() / 1000, '%Y%M%D_%h%m%s') + '.txt'
                );
            }
            kernel.showDialog(
                [
        ...
        exportDiv.append(
            ...
            $('<div class="expOauth">').append(
                ...
                $('<table class="expUpDown">').append(
                    $('<tr>').append($('<td>').append(inServWCA), $('<td>').append(outServWCA))
                )
            ),
            $('<div class="expOauth">').append(
                $('<table id="gglLogin">').append(gglDataTr),
                $('<table class="expUpDown">').append(
                    $('<tr>').append($('<td>').append(inServGGL), $('<td>').append(outServGGL))
                )
            ),
            exportTable
        );
