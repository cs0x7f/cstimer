...
var exportFunc = execMain(function () {
    ...
    var inServ = $('<a class="click"/>')
        .html(EXPORT_FROMSERV + " (csTimer)")
        .click(downloadData);
    var outServ = $('<a class="click"/>')
        .html(EXPORT_TOSERV + " (csTimer)")
        .click(uploadDataClk);

    var outCustomServ = $('<a class="click"/>')
        .html("Export to URL")
        .click(uploadDataCustomServ);
    ...
    exportTable.append(
        ...
        $("<tr>").append($("<td>").append(inServ), $("<td>").append(outServ)),
        $("<tr>").append(
            $("<td colspan=2>").append(
                $('<a class="click"/>')
        ...
        $("<tr>").append($("<td colspan=2>").append(outCustomServ)),
    );

    function updateExpString() {
        return storage.exportAll().then(function (exportObj) {
            exportObj["properties"] = mathlib.str2obj(
    ...
    function loadData(data) {
        ...
        storage
            ...
            .then(function (exportObj) {
                for (
                    var sessionIdx = 1;
                    sessionIdx <= ~~kernel.getProp("sessionN");
                    sessionIdx++
                ) {
                    var times = mathlib.str2obj(
                        exportObj["session" + sessionIdx] || [],
                    );
                    var timesNew = mathlib.str2obj(
                        data["session" + sessionIdx] || [],
                    );
                ...
                if (
                    $.confirm(
                        IMPORT_FINAL_CONFIRM.replace("%d", sessionDelta)
                            .replace("%a", solveAdd)
                            .replace("%r", solveRm),
                    )
                ) {
    ...
    function getId(e) {
        ...
        if (e.target === outServWCA[0] || e.target === inServWCA[0]) {
            ...
            localStorage["locData"] = JSON.stringify({
                id: id,
                compid: getDataId("locData", "compid"),
            });
    ...
    function getLocalDataSliced(id) {
        return storage
            ...
            .then(function (exportObj) {
                var slices = {};
                var baseObj = {};
                baseObj["properties"] = mathlib.str2obj(
                    localStorage["properties"],
                );
                for (var key in exportObj) {
                    ...
                    for (var i = 0; i < times.length; i += 1000) {
                        var timesSlice = LZString.compressToEncodedURIComponent(
                            JSON.stringify(times.slice(i, i + 1000)),
                        );
                ...
                if (id != undefined) {
                    slices[id] = LZString.compressToEncodedURIComponent(
                ...
                ff +
                    ...
                    msgfmt
                        .replace(
                            "%s",
                            nsolv ||
                                (size ? Math.ceil(size / 1024) + " KB" : "N/A"),
                        )
                        .replace(
                            "%t",
                            new Date(files[ff].modifiedTime).toLocaleString(),
                        ),
            );
        }
    ...
    function downloadData(e) {
        ...
        var cntCallback = function (val) {
            ...
            if (kernel.getProp("expp")) {
                idx = promptWhichFile(
                    val["files"] || mathlib.valuedArray(cnt, {}),
                );
                if (idx <= 0 || idx > cnt) {
        ...
        var dataCallback = function (val) {
            ...
            try {
                metaObj = JSON.parse(
                    LZString.decompressFromEncodedURIComponent(val["data"]),
                );
            } catch (err) {
            ...
            for (var key in metaObj) {
                if (
                    ...
                    metaObj[key].length == 0 ||
                    $.isArray(metaObj[key][0])
                ) {
            ...
            if (keys.length > 0) {
                ...
                jobs = jobs
                    ...
                    .then(function (slices) {
                        localSlices = slices;
                        var reqKeys = keys.filter(function (elem) {
                            return !(elem in localSlices);
                        });
                        if (reqKeys.length == 0) {
    ...
    function uploadDataCustomServ() {
        updateExpString().then(function () {
            var url = $.prompt("Enter destination URL");
            if (!url) {
                return;
            }

            var target = outCustomServ;
            var rawText = target.html();
        ...
        updateExpString().then(function () {
            ...
            kernel.showDialog(
                [
                    ...
                    undefined,
                    0,
                    [EXPORT_ONLYOPT, exportProperties],
                    [EXPORT_ACCOUNT, exportAccounts],
                ],
                "export",
                EXPORT_DATAEXPORT,
            );
    ...
    function exportByPrompt(expOpt) {
        var compOpt = LZString.compressToEncodedURIComponent(
            JSON.stringify(expOpt),
        );
    ...
    function updateUserInfoFromWCA() {
        ...
        if (!wcaData["access_token"]) {
            ...
            wcaDataTd.html(
                "WCAID: " + me["wca_id"] + "<br>" + "Name: " + me["name"],
            );
            wcaDataTr
                .click(logoutFromWCA.bind(undefined, true))
                .addClass("click");
    ...
    function updateUserInfoFromGGL() {
        ...
        if (!gglData["access_token"]) {
            ...
            gglDataTr
                .click(logoutFromGGL.bind(undefined, true))
                .addClass("click");
        ...
        if (value[0] == "atexpa") {
            if (value[1] == "id") {
                ...
                if (!isValidId(getDataId("wcaData", "cstimer_token"))) {
                    $.alert(
                        "Please Login with WCA Account in Export Panel First",
                    );
                    kernel.setProp("atexpa", "a");
                    return;
                }
                ...
                if (!getDataId("gglData", "access_token")) {
                    $.alert(
                        "Please Login with Google Account in Export Panel First",
                    );
                    kernel.setProp("atexpa", "a");
                    return;
                }
    ...
    function doBackExport() {
        ...
        updateExpString().then(function () {
            if (atexpa == "id" || atexpa == "wca") {
                var id =
                    atexpa == "id"
                        ? getDataId("locData", "id")
                        : getDataId("wcaData", "cstimer_token");
                if (!isValidId(id)) {
        ...
        if (solvesAfterExport >= kernel.getProp("atexpi", 100)) {
            ...
            if (atexpa == "n") {
                ...
                if (solvesAfterExport % 100 == 0) {
                    logohint.push(
                        EXPORT_AEXPALERT.replace("%d", solvesAfterExport),
                    );
                }
                return;
            }
    ...
    $(function () {
        ...
        kernel.regProp(
            ...
            "atexpi",
            ~1,
            "Auto Export Interval (Solves)",
            [100, [50, 100, 200, 500], ["50", "100", "200", "500"]],
        );
        ...
        if (window.FileReader && window.Blob) {
            ...
            readerOther.onload = function () {
                var n_import = stats.importSessions(
                    TimerDataConverter(this.result),
                );
                if (n_import == 0) {
