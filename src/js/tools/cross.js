...
var cross = execBoth(function(createMove, edgeMove, createPrun, setNPerm, getNPerm, Cnk, getPruning) {
	...
	function solve_cross(scramble, fdiv) {
		init();
		var moves = curScramble = kernel.parseScramble(scramble, "FRUBLD"); // = scramble.split(/\s+/);
		fdiv.empty();
		for (var face = 0; face < 6; face++) {
			...
			span.append(faceStr[face] + "(", clk, "): " + rotIdx[face] + " " + sol.join("&nbsp;") + '<br>')
			fdiv.append(span);
		}
	}
	...
	function ecClick() {
		...
		solve_xcross(curScramble, face, span);
	}

	function solve_xcross(moves, face, span) {
		xinit();
		var flip = 0;
		var perm = 0;
		var e1 = [8, 10, 12, 14];
...
}, function(createMove, edgeMove, createPrun, setNPerm, getNPerm, Cnk, getPruning) {
	var permPrun, flipPrun, ecPrun, fullPrun;
	var cmv = [];
	var pmul = [];
	var fmul = [];
