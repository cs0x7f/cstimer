...
<ul>
<li><strong data="opt_ahide">計時期間隱藏所有物件</strong>. Hide logo and all panels when timing.</li>
<li><strong data="opt_useMilli">精確到毫秒</strong>. Display the millisecond digit, no matter whether it is checked, the internal timing accuracy of csTimer is 1 millisecond.</li>
<li><strong data="opt_timeFormat">時間格式</strong>. Time format to display.</li>
<li><strong data="opt_atexpa">(每100轉)自動匯出</strong>. If checked, csTimer will export the solves automatically per 100 solves to the specified place, local file, csTimer server, or Google Storage.</li>
<li><strong data="opt_expp">載入更舊的數據</strong>. If you've uploaded multiple backups, you can import from one of the up to 10 most recently uploaded backups, if you accidentally upload an empty backup, this option will help you retrieve your solves.</li>
<li><strong data="opt_useLogo">圖標中的提示訊息</strong>. cstimer's Logo will serve as an information display panel that prompts for a variety of information you may be interested in, such as breaking PB.</li>
<li><strong data="opt_showAvg">顯示「平均」標籤</strong>. Two lines of labels are displayed below the the main timer, the current two averages, ao5 and ao12 by default.</li>
<li><strong data="opt_zoom">縮放</strong>. You can adjust sizes of all elements by this option.</li>
<li><strong data="opt_font">計時器字體</strong>. Font of the main timer.</li>
<li><strong data="opt_uidesign">UI設計</strong>. You can switch ui design to material-like, or hide shadows by this option.</li>
<li><strong data="opt_view">UI 介面</strong>. Switch between desktop and mobile views.</li>
<li><strong data="opt_wndScr">打亂面板顯示樣式</strong>. Make scramble panel embedded into background.</li>
<li><strong data="opt_wndStat">統計面板顯示樣式</strong>. Make list times panel embedded into background.</li>
<li><strong data="opt_wndTool">工具面板顯示樣式</strong>. Make tool panel embedded into background.</li>
<li><strong data="opt_bgImgO">背景圖片不透明度</strong>. Opacity of the background image.</li>
<li><strong data="opt_bgImgS">背景圖片</strong>. You can select your own image as the background image, however, only https urls are available due to security constraint of the browser.</li>
<li><strong data="opt_timerSize">計時器大小</strong>. Set the size of main timer.</li>
<li><strong data="opt_smallADP">小數點後使用小型字體</strong>. Use a smaller font size after the digital point in main timer.</li>
<li><strong data="opt_useMouse">使用滑鼠計時</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
<li><strong data="opt_useIns">使用WCA觀察</strong>. Enable WCA inspection procedure, which is a 15-second countdown, auto +2/DNF penalty will also be enabled if you inspecting more than 15 seconds.</li>
<li><strong data="opt_voiceIns">WCA觀察人聲提醒</strong>. Alert at 8s/12s of inspection, to simulate the alert from judge in WCA competitions.</li>
<li><strong data="opt_voiceVol">語音音量</strong>. Voice level of the alert above.</li>
<li><strong data="opt_input">輸入時間使用</strong>. csTimer is able to add solves by several ways, it supports manually input, automatically record from a stackmat timer, connect to a bluetooth smart cube or play virtual Rubik's cube, besides keyboard timing.</li>
<li><strong data="opt_intUN">輸入整數時的單位</strong>. When you type an integer XXX in the input box, what does it mean, XXX second or XXX centisecond or XXX millisecond?</li>
<li><strong data="opt_timeU">時間更新頻率</strong>. How timer is updated when timing.</li>
<li><strong data="opt_preTime">按壓空白鍵時間(秒)</strong>. How long the space bar should be held before the timer turns green.</li>
<li><strong data="opt_phases">多階段計時</strong>. Number of phases, press any key to mark a split point when timing.</li>
<li><strong data="opt_stkHead">使用stackmat狀態資訊</strong>. Stackmat will report its state, e.g. whether left or right area is touched, then csTimer is able to use these information, however, the data error might occur and cause unexpected behavior.</li>
<li><strong data="opt_scrSize">打亂字體大小</strong>. Size of the scramble text.</li>
<li><strong data="opt_scrASize">自動調整打亂字體大小</strong>. The size of the scramble text will be automatically adjusted by the length of the scramble, which works with together previous option.</li>
<li><strong data="opt_scrMono">等寬字體打亂</strong>. Use monospaced font for scramble text.</li>
<li><strong data="opt_scrLim">限制打亂區高度</strong>. When the scramble area is too high, a scroll bar will occur to avoid the raising of the scramble panel.</li>
<li><strong data="opt_scrAlign">打亂對齊</strong>. Alignment of the whole scramble area, include scramble type selector.</li>
...
<li><strong data="opt_scrFast">4x4x4使用快速打亂(非官方)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
<li><strong data="opt_scrKeyM">打亂中標記關鍵步驟</strong>. Mark a key move in the scramble, e.g. the move that take the state away from square shape in SQ1 scrambles.</li>
<li><strong data="opt_scrClk">點擊打亂時的動作</strong>. Behavior when you click on the scramble text, copy scramble or generate next scramble.</li>
<li><strong data="opt_trim">刪減數據中的資料數</strong>. Number of solves trimmed at head and tail of solves when calculating average.</li>
<li><strong data="opt_statsum">在時間清單前顯示摘要</strong>. Show the statistics table before time list.</li>
<li><strong data="opt_printScr">在統計列表中顯示打亂</strong>. Print scramble in round statistics dialog.</li>
<li><strong data="opt_printDate">在統計列表中顯示日期</strong>. Print solving date in round statistics dialog.</li>
<li><strong data="opt_imrename">創建後立即重新命名階段</strong>. Immediately rename a session after creating it.</li>
<li><strong data="opt_scr2ss">更換打亂模式時創建新階段</strong>. When switching scramble type, a new session will be created.</li>
<li><strong data="opt_statinv">顛倒時間清單</strong>. Invert the time list, thus, latest solves will at the bottom of the time list.</li>
<li><strong data="opt_statclr">允許階段空白</strong>. When disabled, an '+' button (for session creating) will replace the 'X' button besides the session selector, thus, when clicked, a new empty session will be created instead of clearing the whole session.</li>
