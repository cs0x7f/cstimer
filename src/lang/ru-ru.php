...
<ul>
<li><strong data="opt_ahide">скрывать все элементы когда идёт время</strong>. Hide logo and all panels when timing.</li>
<li><strong data="opt_useMilli">использовать миллисекунды</strong>. Display the millisecond digit, no matter whether it is checked, the internal timing accuracy of csTimer is 1 millisecond.</li>
...
<li><strong data="opt_uidesign">Дизайн интерфейса</strong>. You can switch ui design to material-like, or hide shadows by this option.</li>
<li><strong data="opt_view">Стиль интерфейса</strong>. Switch between desktop and mobile views.</li>
<li><strong data="opt_wndScr">Стиль панели со скрамблом</strong>. Make scramble panel embedded into background.</li>
<li><strong data="opt_wndStat">Стиль панели статистики</strong>. Make list times panel embedded into background.</li>
<li><strong data="opt_wndTool">Стиль отображения панели инструментов</strong>. Make tool panel embedded into background.</li>
<li><strong data="opt_bgImgO">прозрачность фонового изображения</strong>. Opacity of the background image.</li>
<li><strong data="opt_bgImgS">фоновое изображение</strong>. You can select your own image as the background image, however, only https urls are available due to security constraint of the browser.</li>
<li><strong data="opt_timerSize">размер таймера</strong>. Set the size of main timer.</li>
<li><strong data="opt_smallADP">использовать маленький шрифт после десятичной точки</strong>. Use a smaller font size after the digital point in main timer.</li>
<li><strong data="opt_useMouse">использовать таймер мышкой</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
<li><strong data="opt_useIns">использовать WCA инспекцию</strong>. Enable WCA inspection procedure, which is a 15-second countdown, auto +2/DNF penalty will also be enabled if you inspecting more than 15 seconds.</li>
<li><strong data="opt_voiceIns">голосовое предупреждение при инспекции</strong>. Alert at 8s/12s of inspection, to simulate the alert from judge in WCA competitions.</li>
<li><strong data="opt_voiceVol">Громкость голоса</strong>. Voice level of the alert above.</li>
<li><strong data="opt_input">вставлять время с</strong>. csTimer is able to add solves by several ways, it supports manually input, automatically record from a stackmat timer, connect to a bluetooth smart cube or play virtual Rubik's cube, besides keyboard timing.</li>
...
<li><strong data="opt_timeU">обновление таймера</strong>. How timer is updated when timing.</li>
<li><strong data="opt_preTime">время пространства(секунд)</strong>. How long the space bar should be held before the timer turns green.</li>
<li><strong data="opt_phases">число этапов</strong>. Number of phases, press any key to mark a split point when timing.</li>
<li><strong data="opt_stkHead">Использовать информацию о статусе Stackmat</strong>. Stackmat will report its state, e.g. whether left or right area is touched, then csTimer is able to use these information, however, the data error might occur and cause unexpected behavior.</li>
<li><strong data="opt_scrSize">размер скрамбла</strong>. Size of the scramble text.</li>
...
<li><strong data="opt_scrMono">моноширинный скрамбл</strong>. Use monospaced font for scramble text.</li>
<li><strong data="opt_scrLim">Ограничить высоту зоны скрамбла</strong>. When the scramble area is too high, a scroll bar will occur to avoid the raising of the scramble panel.</li>
<li><strong data="opt_scrAlign">Выравнивание зоны скрамбла</strong>. Alignment of the whole scramble area, include scramble type selector.</li>
...
<li><strong data="opt_scrFast">Использовать быстрый скрамбл для 4х4х4(неофициально)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
...
<li><strong data="opt_statsum">как суммарный после списка времени</strong>. Show the statistics table before time list.</li>
