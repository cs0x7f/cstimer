<h1>csTimer versió <?php echo $version;?> - Cronometre Professional d'entrenament per a Speedcubing</h1>
...
<li><strong data="opt_atexpa">Exportar automàticament (per cada 100 resolucions)</strong>. If checked, csTimer will export the solves automatically per 100 solves to the specified place, local file, csTimer server, or Google Storage.</li>
<li><strong data="opt_expp">Importar dades no més recents</strong>. If you've uploaded multiple backups, you can import from one of the up to 10 most recently uploaded backups, if you accidentally upload an empty backup, this option will help you retrieve your solves.</li>
<li><strong data="opt_useLogo">missatges de pista al logo</strong>. csTimer's Logo will serve as an information display panel that prompts for a variety of information you may be interested in, such as breaking PB.</li>
<li><strong data="opt_showAvg">Mostrar Mitjanes</strong>. Two lines of labels are displayed below the the main timer, the current two averages, ao5 and ao12 by default.</li>
...
<li><strong data="opt_uidesign">Disseny d&#x27;interfície d&#x27;usuari és</strong>. You can switch ui design to material-like, or hide shadows by this option.</li>
...
<li><strong data="opt_bgImgO">opacitat de la imatge de fons</strong>. Opacity of the background image.</li>
<li><strong data="opt_bgImgS">imatge de fons</strong>. You can select your own image as the background image, however, only https urls are available due to security constraint of the browser.</li>
...
<li><strong data="opt_color">Seleciona tema de color</strong>. Select color schemes of csTimer. Click csTimer's logo to show more color schemes.</li>
...
<li><strong data="opt_voiceVol">Volum de Veu</strong>. Voice level of the alert above.</li>
...
<li><strong data="opt_intUN">Unitat en registrar un nombre enter</strong>. When you type an integer XXX in the input box, what does it mean, XXX second or XXX centisecond or XXX millisecond?</li>
...
<li><strong data="opt_stkHead">Utilitzar Informació d&#x27;Estat de Stackmat</strong>. Stackmat will report its state, e.g. whether left or right area is touched, then csTimer is able to use these information, however, the data error might occur and cause unexpected behavior.</li>
<li><strong data="opt_scrSize">Mida de la barreja</strong>. Size of the scramble text.</li>
<li><strong data="opt_scrASize">Tamany automàtic de barreja</strong>. The size of the scramble text will be automatically adjusted by the length of the scramble, which works with together previous option.</li>
...
<li><strong data="opt_scrClk">Acció a la hora de fer clic a la barreja</strong>. Behavior when you click on the scramble text, copy scramble or generate next scramble.</li>
<li><strong data="opt_trim">Nombre de resolucions no contants a cada costat</strong>. Number of solves trimmed at head and tail of solves when calculating average.</li>
<li><strong data="opt_statsum">mostrar resum abans de llista de temps</strong>. Show the statistics table before time list.</li>
<li><strong data="opt_statthres">Mostrar objectiu de temps per al millor de sessió</strong>. In the statistics table, the time required to refresh personal best after next solve is displayed. "N/A" means the next solve will not refresh PB no matter how fast it is, "&#8734;" means any time except DNF will refresh PB.</li>
<li><strong data="opt_printScr">imprimir barreja/es a estadístiques</strong>. Print scramble in round statistics dialog.</li>
<li><strong data="opt_printDate">imprimir data de resolució a estadístiques</strong>. Print solving date in round statistics dialog.</li>
<li><strong data="opt_imrename">canviar el nom de la sessió immediatament després de creació</strong>. Immediately rename a session after creating it.</li>
<li><strong data="opt_scr2ss">crear nova sessió després de canviar tipus de barreja</strong>. When switching scramble type, a new session will be created.</li>
<li><strong data="opt_statinv">Llista de temps invers</strong>. Invert the time list, thus, latest solves will at the bottom of the time list.</li>
<li><strong data="opt_statclr">Opció de buidar sessions</strong>. When disabled, an '+' button (for session creating) will replace the 'X' button besides the session selector, thus, when clicked, a new empty session will be created instead of clearing the whole session.</li>
<li><strong data="opt_absidx">Mostrar índex absolut a l&#x27;informe d&#x27;estadístiques</strong>. Show absolute index in the session instead of 1 to number of solves (e.g. 1/2/3 for mo3) in round statistics.</li>
<li><strong data="opt_rsfor1s">Mostrar estadístiques quan es fa clic al número de la resolució</strong>. When click the first row of the time list, show a round statistics for a single solve.</li>
<li><strong data="opt_statal">Indicadors d&#x27;estadística</strong>. Statistical indicator for the statistics table, when customizing, aoX and moX are available.</li>
...
<li><strong data="opt_disPrec">precisió de distribució de temps</strong>. Time interval for the time distribution tool.</li>
<li><strong data="opt_solSpl">Mostrar resolució progressivament</strong>. If selected, only the length of a solution from a solver is displayed, and you can view the solution one move by one move, otherwise, the whole solution is displayed.</li>
...
<li><strong data="opt_useKSC">Fer servir dreceres del teclat</strong>. Use keyboard shortcut to switch scramble type, generate next scramble, switch between sessions. Click csTimer's logo to show details.</li>
...
<h2>Tools detail</h2>
<ul>
<li><strong data="tool_scrgen">Generador de Barrejes</strong>. You are able to generate up to 999 scrambles with one click by this tool.</li>
...
<li><strong data="tool_roux1">Solucionadors &gt; Solucionar S1 Roux</strong>. Roux 1st step solver, which solves a 1x2x3 block.</li>
<li><strong data="tool_eoline">Solucionadors &gt; Solucionar línia EO</strong>. EO line solver, which solves orientations of all 12 edges, and positions of DF and DB edges.</li>
<li><strong data="tool_cross">Solucionadors &gt; Solucionar creu</strong>. Cross solver, which solve DF, DL, DR, DB edges.</li>
<li><strong data="tool_222face">Solucionadors &gt; Cara de 2x2x2</strong>. 2x2x2 face solver, which solves a face of 2x2x2 cube.</li>
<li><strong data="tool_333cf">Solucionadors &gt; Cross + F2L</strong>. Cross and F2L solver, which solves Cross and 4 F2Ls with computer search, so the solution might be far from human solutions.</li>
<li><strong data="tool_333roux">Solucionadors &gt; Roux S1 + S2</strong>. Roux 1st and 2nd step solver, which firstly solves a 1x2x3 block on the left face and then expend another 1x2x3 block on the right face with R, M, r, U.</li>
<li><strong data="tool_333petrus">Solucionadors &gt; 2x2x2 + 2x2x3</strong>. Petrus 1st and 2nd step solver, which firstly solves an 2x2x2 block on the left and then expend it to a 2x2x3 on the left.</li>
<li><strong data="tool_333zz">Solucionadors &gt; EOLine + ZZF2L</strong>. Eoline and ZZF2L solver, which firstly solves the EOLine and then solve one of left 1x2x3 or right 1x2x3 and the solve the other 2x2x3.</li>
<li><strong data="tool_sq1cs">Solucionadors &gt; SQ1 S1 + S2</strong>. SQ1 1st and 2nd step solver, which firstly solves the shape of SQ1 and then split U pieces and D pieces.</li>
<li><strong data="tool_pyrv">Solucionadors &gt; Pyraminx V</strong>. Pyraminx V solver, which solves three corners and two edges to shape into a 'V' pattern for pyraminx.</li>
<li><strong data="tool_skbl1">Solucionadors &gt; Skewb Face</strong>. Skewb face solver, which solves a layer of skewb, more specifically, 1 center and 4 neighbor corners.</li>
...
<li><strong data="tool_syncseed">Barreja Comuna</strong>. Using same scrambles with friends by setting a common seed.</li>
