...
<ul>
<li><strong data="opt_ahide">記録中はすべての要素を非表示にする</strong>. Hide logo and all panels when timing.</li>
<li><strong data="opt_useMilli">ミリ秒を使用</strong>. Display the millisecond digit, no matter whether it is checked, the internal timing accuracy of csTimer is 1 millisecond.</li>
<li><strong data="opt_timeFormat">時間形式</strong>. Time format to display.</li>
<li><strong data="opt_atexpa">オートエクスポート (100ソルブごと)</strong>. If checked, csTimer will export the solves automatically per 100 solves to the specified place, local file, csTimer server, or Google Storage.</li>
<li><strong data="opt_expp">最新でないデータをインポート</strong>. If you've uploaded multiple backups, you can import from one of the up to 10 most recently uploaded backups, if you accidentally upload an empty backup, this option will help you retrieve your solves.</li>
<li><strong data="opt_useLogo">ロゴにメッセージヒントを表示</strong>. cstimer's Logo will serve as an information display panel that prompts for a variety of information you may be interested in, such as breaking PB.</li>
<li><strong data="opt_showAvg">アベレージのラベルを表示</strong>. Two lines of labels are displayed below the the main timer, the current two averages, ao5 and ao12 by default.</li>
<li><strong data="opt_zoom">拡大</strong>. You can adjust sizes of all elements by this option.</li>
<li><strong data="opt_font">タイマーフォントの選択</strong>. Font of the main timer.</li>
<li><strong data="opt_uidesign">UIデザイン</strong>. You can switch ui design to material-like, or hide shadows by this option.</li>
<li><strong data="opt_view">UIスタイル</strong>. Switch between desktop and mobile views.</li>
<li><strong data="opt_wndScr">スクランブルパネルの表示スタイル</strong>. Make scramble panel embedded into background.</li>
<li><strong data="opt_wndStat">統計パネルの表示スタイル</strong>. Make list times panel embedded into background.</li>
<li><strong data="opt_wndTool">ツールパネルの表示スタイル</strong>. Make tool panel embedded into background.</li>
<li><strong data="opt_bgImgO">背景画像の透過度</strong>. Opacity of the background image.</li>
<li><strong data="opt_bgImgS">背景画像</strong>. You can select your own image as the background image, however, only https urls are available due to security constraint of the browser.</li>
<li><strong data="opt_timerSize">タイマー サイズ</strong>. Set the size of main timer.</li>
<li><strong data="opt_smallADP">小数点の後に小さいフォントを使用する</strong>. Use a smaller font size after the digital point in main timer.</li>
<li><strong data="opt_useMouse">マウスタイマーの使用</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
<li><strong data="opt_useIns">WCA インスペクションの使用</strong>. Enable WCA inspection procedure, which is a 15-second countdown, auto +2/DNF penalty will also be enabled if you inspecting more than 15 seconds.</li>
<li><strong data="opt_voiceIns">WCA インスペクションの音声警告</strong>. Alert at 8s/12s of inspection, to simulate the alert from judge in WCA competitions.</li>
<li><strong data="opt_voiceVol">音量</strong>. Voice level of the alert above.</li>
<li><strong data="opt_input">タイム入力方法</strong>. csTimer is able to add solves by several ways, it supports manually input, automatically record from a stackmat timer, connect to a bluetooth smart cube or play virtual Rubik's cube, besides keyboard timing.</li>
<li><strong data="opt_intUN">整数を入力するときの単位</strong>. When you type an integer XXX in the input box, what does it mean, XXX second or XXX centisecond or XXX millisecond?</li>
<li><strong data="opt_timeU">タイマー更新間隔</strong>. How timer is updated when timing.</li>
<li><strong data="opt_preTime">スペースキーを押し続ける時間(秒)</strong>. How long the space bar should be held before the timer turns green.</li>
<li><strong data="opt_phases">マルチフェイズ</strong>. Number of phases, press any key to mark a split point when timing.</li>
<li><strong data="opt_stkHead">スタックマットのステータス情報を利用する</strong>. Stackmat will report its state, e.g. whether left or right area is touched, then csTimer is able to use these information, however, the data error might occur and cause unexpected behavior.</li>
<li><strong data="opt_scrSize">スクランブルサイズ</strong>. Size of the scramble text.</li>
<li><strong data="opt_scrASize">自動スクランブルのサイズ</strong>. The size of the scramble text will be automatically adjusted by the length of the scramble, which works with together previous option.</li>
<li><strong data="opt_scrMono">等幅スクランブル</strong>. Use monospaced font for scramble text.</li>
<li><strong data="opt_scrLim">スクランブル領域の高さ制限</strong>. When the scramble area is too high, a scroll bar will occur to avoid the raising of the scramble panel.</li>
<li><strong data="opt_scrAlign">スクランブルエリアの左右配置</strong>. Alignment of the whole scramble area, include scramble type selector.</li>
...
<li><strong data="opt_scrFast">4x4x4の高速スクランブルを利用する(非公式)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
<li><strong data="opt_scrKeyM">スクランブルの主要な動きにラベルをつける</strong>. Mark a key move in the scramble, e.g. the move that take the state away from square shape in SQ1 scrambles.</li>
<li><strong data="opt_scrClk">スクランブルをクリックしたときの動作</strong>. Behavior when you click on the scramble text, copy scramble or generate next scramble.</li>
<li><strong data="opt_trim">片側で除外するソルブ数</strong>. Number of solves trimmed at head and tail of solves when calculating average.</li>
<li><strong data="opt_statsum">タイム一覧の前にサマリーを表示</strong>. Show the statistics table before time list.</li>
<li><strong data="opt_printScr">統計情報にスクランブルを表示</strong>. Print scramble in round statistics dialog.</li>
<li><strong data="opt_printDate">統計情報にソルブ日時を表示</strong>. Print solving date in round statistics dialog.</li>
<li><strong data="opt_imrename">セッション作成後すぐに名前を変更する</strong>. Immediately rename a session after creating it.</li>
<li><strong data="opt_scr2ss">スクランブルのタイプを変更したとき新しいセッションを作成</strong>. When switching scramble type, a new session will be created.</li>
<li><strong data="opt_statinv">逆順のタイム一覧</strong>. Invert the time list, thus, latest solves will at the bottom of the time list.</li>
...
<li><strong data="opt_giiAED">ハードウェアエラーの自動検知</strong>. Some bluetooth cubes will loss some of moves due to hardware failure, csTimer will try to detect such hardware.</li>
</ul>
<h2>Tools detail</h2>
<ul>
<li><strong data="tool_scrgen">スクランブルジェネレーター</strong>. TO_BE_FILLED</li>
<li><strong data="tool_cfm">タイム確認</strong>. TO_BE_FILLED</li>
<li><strong data="tool_hugestats">クロスセッションの統計情報</strong>. TO_BE_FILLED</li>
<li><strong data="tool_stats">統計情報</strong>. TO_BE_FILLED</li>
<li><strong data="tool_distribution">タイム分布</strong>. TO_BE_FILLED</li>
<li><strong data="tool_trend">タイムトレンド</strong>. TO_BE_FILLED</li>
<li><strong data="tool_dlystat">Daily Statistics</strong>. TO_BE_FILLED</li>
<li><strong data="tool_image">スクランブルの描画</strong>. TO_BE_FILLED</li>
<li><strong data="tool_roux1">ソルバー &gt; Roux S1 ソルブ</strong>. TO_BE_FILLED</li>
<li><strong data="tool_eoline">ソルバー &gt; EOLineソルブ</strong>. TO_BE_FILLED</li>
<li><strong data="tool_cross">ソルバー &gt; クロスソルブ</strong>. TO_BE_FILLED</li>
<li><strong data="tool_222face">ソルバー &gt; 2x2x2 一面ソルブ</strong>. TO_BE_FILLED</li>
<li><strong data="tool_333cf">ソルバー &gt; Cross + F2L</strong>. TO_BE_FILLED</li>
<li><strong data="tool_333roux">ソルバー &gt; Roux S1 + S2</strong>. TO_BE_FILLED</li>
<li><strong data="tool_333petrus">ソルバー &gt; 2x2x2 + 2x2x3</strong>. TO_BE_FILLED</li>
<li><strong data="tool_333zz">ソルバー &gt; EOLine + ZZF2L</strong>. TO_BE_FILLED</li>
<li><strong data="tool_sq1cs">ソルバー &gt; SQ1 S1 + S2</strong>. TO_BE_FILLED</li>
<li><strong data="tool_pyrv">ソルバー &gt; Pyraminx V</strong>. TO_BE_FILLED</li>
<li><strong data="tool_skbl1">ソルバー &gt; Skewb Face</strong>. TO_BE_FILLED</li>
<li><strong data="tool_giikerutil">Giiker キューブ</strong>. TO_BE_FILLED</li>
<li><strong data="tool_if">InsertionFinder</strong>. TO_BE_FILLED</li>
<li><strong data="tool_mtrnm">メトロノーム</strong>. TO_BE_FILLED</li>
<li><strong data="tool_onlinecomp">オンライン大会</strong>. TO_BE_FILLED</li>
