...
<ul>
<li><strong data="opt_ahide">Göm alla element när timern tar tid</strong>. Hide logo and all panels when timing.</li>
<li><strong data="opt_useMilli">använd millisekunder</strong>. Display the millisecond digit, no matter whether it is checked, the internal timing accuracy of csTimer is 1 millisecond.</li>
<li><strong data="opt_timeFormat">tidsformat</strong>. Time format to display.</li>
...
<li><strong data="opt_useLogo">Tipsmeddelanden i logotypen</strong>. csTimer's Logo will serve as an information display panel that prompts for a variety of information you may be interested in, such as breaking PB.</li>
<li><strong data="opt_showAvg">Visa avergage markering</strong>. Two lines of labels are displayed below the the main timer, the current two averages, ao5 and ao12 by default.</li>
<li><strong data="opt_zoom">Zooma</strong>. You can adjust sizes of all elements by this option.</li>
<li><strong data="opt_font">använd timerns teckensnitt</strong>. Font of the main timer.</li>
...
<li><strong data="opt_useMouse">använd mus timer</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
...
<li><strong data="opt_stkHead">Använd Stackmatstatusinformation</strong>. Stackmat will report its state, e.g. whether left or right area is touched, then csTimer is able to use these information, however, the data error might occur and cause unexpected behavior.</li>
<li><strong data="opt_scrSize">blandningsstorlek</strong>. Size of the scramble text.</li>
...
<li><strong data="opt_scrMono">fastbredd blandning</strong>. Use monospaced font for scramble text.</li>
...
<li><strong data="opt_scrFast">Använd snabb blandning för 4x4x4 (ej officiell)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
<li><strong data="opt_scrKeyM">Markeringsnyckeln flyttas i blandning</strong>. Mark a key move in the scramble, e.g. the move that take the state away from square shape in SQ1 scrambles.</li>
...
<li><strong data="opt_printScr">skriv ut blandning(ar) i statistik</strong>. Print scramble in round statistics dialog.</li>
<li><strong data="opt_printDate">skriv ut lösningens datum i statistik</strong>. Print solving date in round statistics dialog.</li>
...
<li><strong data="opt_scr2ss">skapa ny session när ny blandningstyp väljs</strong>. When switching scramble type, a new session will be created.</li>
...
<li><strong data="opt_delmul">Aktivera flera raderingar</strong>. Able to delete multiple solves starts from a solve, for avoid misunderstand, the selected solve will be the oldest solve to delete.</li>
...
<li><strong data="opt_imgSize">Blandningsbildstorlek</strong>. Set the size of scramble image.</li>
<li><strong data="opt_NTools">antal verktyg</strong>. csTimer is able to show up to 4 tools simultaneously.</li>
<li><strong data="opt_useKSC">använd tangentborsgenväg</strong>. Use keyboard shortcut to switch scramble type, generate next scramble, switch between sessions, etc.</li>
...
<li><strong data="opt_vrcMP">flera faser</strong>. Automatic multi-phase split for virtual Rubik's cube and bluetooth cube.</li>
...
<li><strong data="opt_giiAED">Automatisk identifiering av hårdvarafel</strong>. Some bluetooth cubes will loss some of moves due to hardware failure, csTimer will try to detect such case.</li>
</ul>
<h2>Tools detail</h2>
<ul>
<li><strong data="tool_scrgen">Blandningsgenerator</strong>. You are able to generate up to 999 scrambles with one click by this tool.</li>
...
<li><strong data="tool_distribution">tidsfördelning</strong>. Time distribution and stability analysis, &lt;X Y/Z means there are totally Y solves less than X seconds, and all of the latest Z solves are less than X seconds in the session.</li>
<li><strong data="tool_trend">tidsutveckling</strong>. Shows a trend curve of all solves in current session.</li>
<li><strong data="tool_dlystat">Daily Statistics</strong>. Count number of solves each day/week/month/year.</li>
<li><strong data="tool_image">rita blandning</strong>. Scramble image to verify a correct scramble, all WCA puzzles are supported.</li>
<li><strong data="tool_roux1">Solvers &gt; lös Roux S1</strong>. Roux 1st step solver, which solves a 1x2x3 block.</li>
<li><strong data="tool_eoline">Solvers &gt; lös EOLine</strong>. EO line solver, which solves orientations of all 12 edges, and positions of DF and DB edges.</li>
<li><strong data="tool_cross">Solvers &gt; lös kors</strong>. Cross solver, which solve DF, DL, DR, DB edges.</li>
...
<li><strong data="tool_onlinecomp">Onlinetävling</strong>. Online competition, so you can login with WCA account and compete with all speedsolvers around the world with same scrambles.</li>
